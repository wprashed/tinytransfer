 ...
 * @plugin
 * @name Core
 * @description UI Library core. Required for all plugins.
 */
...
(function(factory) {
            ...
            Functions = {
                ...
                lockViewport: function(plugin_namespace) {
                    ...
                    if (!$.isEmptyObject(ViewportLocks) && !ViewportLocked) {
                        ...
                        UI.$body.on(Events.gestureChange, Functions.killGesture)
                ...
                unlockViewport: function(plugin_namespace) {
                    ...
                    if ($.isEmptyObject(ViewportLocks) && ViewportLocked) {
                        ...
                        UI.$body.off(Events.gestureChange)
                ...
                decodeEntities: function(string) {
                    ...
                    var el = UI.document.createElement("textarea");
                    el.innerHTML = string;
         ...
         * @example UI.NoConflict();
         */
        Core.prototype.NoConflict = function() {
            UI.DontConflict = true;

            for (var i in UI.Plugins) {
                if (UI.Plugins.hasOwnProperty(i)) {
                    $[i] = UI.Conflicts[i];
                    $.fn[i] = UI.Conflicts.fn[i];
                }
            }
        };
        ...
        Core.prototype.Ready = function(fn) {
            if (
                ...
                (UI.document.readyState !== "loading" && !UI.document.documentElement.doScroll)
            ) {
                fn();
            } else {
                UI.document.addEventListener("DOMContentLoaded", fn);
            }
        };
         ...
         * @example UI.Plugin("namespace", { ... });
         */
        Core.prototype.Plugin = function(namespace, settings) {
            UI.Plugins[namespace] = (function(namespace, settings) {

                var namespaceDash = "ui-" + namespace,
                    namespaceClean = "ui" + namespace.replace(/(^|\s)([a-z])/g, function(m, p1, p2) {
                ...
                if (settings.widget) {
                    ...
                    UI.Conflicts.fn[namespace] = $.fn[namespace];

                    // Widget Delegation: $(".target").uiPlugin("method", ...);
                ...
                UI.Conflicts[namespace] = $[namespace];

                // Utility Delegation: $.uiPlugin("method", ... );
                ...
                if (settings.methods._resize) {
                    ...
                    UI.ResizeHandlers.sort(sortPriority);
                }
                ...
                if (settings.methods._raf) {
                    ...
                    // Sort handlers on push
                    UI.RAFHandlers.sort(sortPriority);
                }

                return settings;
            })(namespace, settings);

            return UI.Plugins[namespace];
        };
        ...
        function setBrowserPrefixes() {
            ...
            UI.transition = transitionProperty;

            for (i in transformProperties) {
                if (transformProperties.hasOwnProperty(i) && transformProperties[i] in testDiv.style) {
                    UI.support.transform = true;
                    transformProperty = transformProperties[i];
                    break;
                }
            }

            UI.transform = transformProperty;
        }

        // Window resize
        function onWindowResize() {
            UI.windowWidth = UI.$window.width();
            UI.windowHeight = UI.$window.height();

            ResizeTimer = Functions.startTimer(ResizeTimer, Debounce, handleWindowResize);
        }

        function handleWindowResize() {
            for (var i in UI.ResizeHandlers) {
                if (UI.ResizeHandlers.hasOwnProperty(i)) {
                    UI.ResizeHandlers[i].callback.call(window, UI.windowWidth, UI.windowHeight);
                }
            }
        }
        UI.$window.on("resize.ui", onWindowResize);
        onWindowResize();

        // RAF
        function handleRAF() {
            if (UI.support.raf) {
                UI.window.requestAnimationFrame(handleRAF);

                for (var i in UI.RAFHandlers) {
                    if (UI.RAFHandlers.hasOwnProperty(i)) {
                        UI.RAFHandlers[i].callback.call(window);
                    }
                }
            }
        }
        ...
        UI.Ready(function() {
            ...
            $("html").addClass((UI.support.touch) ? "touchevents" : "no-touchevents");
