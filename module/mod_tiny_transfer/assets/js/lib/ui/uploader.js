...
(function(factory) {
        ...
        function construct(data) {
            if (UI.support.file) {
                ...
                this.addClass(data.baseClasses).append(html);
         ...
         * @example $(".target").upload("abort", [fid]);
         */

        function abortUpload(data, fid) {
            var file;
            ...
            for (var i in data.queue) {
                if (data.queue.hasOwnProperty(i)) {
                    ...
                    if ($.type(fid) === "undefined" || (fid >= 0 && file.fid === fid)) {
                        // Abort all
        ...
        function handleUpload(data, files) {
            ...
            if (numFiles > 0) {
                ...
                data.queue = data.queue.concat(newFiles);
                data.queue.forEach(function(v, i) {
                    v.index = i;
                });
        ...
        function continueUpload(data) {
            data.stop = false;
            uploadFile(data, data.queue_now);
        }

        // remove
        function removeUpload(data, fid) {
            const i = data.queue.findIndex(v => v.fid === fid);
            data.queue.splice(i, 1);
            data.queue.forEach(function(v, i) {
                v.index = i;
            });
            data.$el.trigger(Events.fileRemove, [fid, data.queue]);

        }
        // add queue
        function addQueueUpload(data, files) {
            data.queue = files;
            continueUpload(data);

        }
        // empty queue
        function emptyQueueUpload(data) {
            data.queue = [];
            stopUpload(data);
        }
        ...
        function uploadFile(data, file) {
            ...
            if (file.size >= data.maxSize || file.error === true) {
                ...
                if (file.currentChunk == undefined) {
                    file.currentChunk = 0;
                }
        ...
        var Plugin = UI.Plugin("uploader", {
                 ...
                 * @param hiddenInput [boolean] <false> "Hide input, add input manually"
                 * @param postData [object] "Extra data to post with upload"
                 * @param postKey [string] <'file'> "Key to upload file as"
                 * @param theme [string] <"ui-light"> "Theme class name"
                 */
                ...
                methods: {
                    ...
                    remove: removeUpload,
                    addQueue: addQueueUpload,
                    emptyQueue: emptyQueueUpload,
                    setData: setData,
                    start: startUpload
                }
            }),
