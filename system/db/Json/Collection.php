...
class Collection
{
    ...
    public function loadData()
    {
        ...
        if (!file_exists($this->filepath)) {
            ...
            if (is_null($data)) {
                throw new \Exception("Failed to load data. File '{$this->filepath}' contain invalid JSON format.");
            }
        }
    ...
    public function execute(Query $query, $type, $arg = null)
    {
        if ($query->getCollection() != $this) {
            throw new \Exception("Cannot execute query. Query is for different collection");
        }
    ...
    protected function save(array $data)
    {
        if ($this->isModeTransaction()) {
            ...
            if (!is_dir($dir)) {
                throw new \Exception("Cannot save database. Directory {$dir} not found or it is not directory.");
            }
